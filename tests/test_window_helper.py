...
class WindowHelperTest(unittest.TestCase):
    def setUp(self):
        ...
        yabai = root/'yabai'
        yabai.write_text('''#!/usr/bin/python3
import json, os, sys
...
''')
        ...
        source = Path(os.environ.get('TEST_HELPER_SOURCE', str(Path.home()/'.config/skhd/win-dir.sh'))).read_text()
        source = source.replace('YABAI="/Users/droy-/dev/yabai-macos27/bin/yabai"',f'YABAI="{yabai}"')
        self.helper.write_text(source)
        self.env = dict(os.environ, TEST_CALLS=str(self.log))
